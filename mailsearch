...
host -t a $(cat mail.txt | sort -R | head -n 1) | grep 'has address' | cut -d ' ' -f 4 | grep -v -F : | sort -R | head -n 1 > unknown.txt
fi
...
#using whois result and turn it to mail.txt format
    ...
    cat whois.txt | grep -F -A 1 'contact:'| tail -n 1 | cut -d '@' -f 2 | cut -d ' ' -f 1 > mail.txt
    fi
    if [[ $(cat mail.txt | wc -l) -eq 0 ]] ; then
